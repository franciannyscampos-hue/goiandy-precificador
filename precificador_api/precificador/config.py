...
class CatalogConfig:
    ...
    @classmethod
    def from_dict(cls, d: dict, nome_padrao: str = 'calibrada') -> "CatalogConfig":
        """Constroi a partir de um dict que pode vir incompleto ou com chaves
        extras (ex: JSON salvo pela tela de calibracao) -- ignora o que nao
        reconhece e usa o padrao pro que faltar, em vez de dar erro."""
        campos_validos = {f.name for f in dataclass_fields(cls)}
        filtrado = {k: v for k, v in d.items() if k in campos_validos and v is not None}
        filtrado.setdefault('nome', nome_padrao)
        filtrado.setdefault('ref_regex', '')
        return cls(**filtrado)
