    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i in indices:
            if (pdf.pages[i].extract_text() or '').strip():
                com_texto += 1
        # se pelo menos 20% das paginas amostradas tem texto, trata como nativo
        # (paginas de capa/separador sem texto sao normais e nao devem enganar)
...
def testar_amostra(pdf_path: str, config, lookup, out_path: str, n_paginas=15) -> Relatorio:
    """Roda so uma amostra (as N primeiras paginas com produto, nao necessariamente
    as N primeiras do arquivo -- pula paginas de capa/indice que nao tem nenhuma
    REF reconhecida) -- pra dar feedback rapido na hora de calibrar uma industria,
    sem precisar processar o catalogo inteiro (que pode ter centenas de paginas)."""
    config.excel_lookup = lookup
    relatorio = Relatorio()
    reader = PdfReader(pdf_path)
    ...
    with pdfplumber.open(pdf_path) as doc:
        limite_absoluto = min(n_total, 60)  # trava mesmo se a config estiver toda errada (0 acertos)
        for i in range(limite_absoluto):
            if paginas_com_produto >= n_paginas:
                break
            page_plumber = doc.pages[i]
            antes = relatorio.encontrados
            if escaneado:
                _processar_pagina_ocr(pdf_path, i, page_plumber, reader, writer, config, relatorio)
            else:
                _processar_pagina_texto(page_plumber, reader, writer, config, relatorio)
            if relatorio.encontrados > antes:
